...
const EnhancedRootAnalyzer = {
    ...
    async analyzeWithAI(word) {
        ...
        // 检查API密钥是否可用
        if (typeof AIRootAnalyzer !== 'undefined' && AIRootAnalyzer.getApiKey()) {
            try {
                console.log('尝试使用AI分析:', wordLower);
                const aiResult = await AIRootAnalyzer.analyzeWithAI(wordLower);
                console.log('AI分析成功:', aiResult);
                if (aiResult && (aiResult.roots || aiResult.length > 0)) {
                    return { result: aiResult.roots || aiResult, source: 'AI', etymology: aiResult.etymology, related: aiResult.related };
                }
            } catch (error) {
                console.log('AI分析失败，使用本地结果:', error.message);
            }
        } else {
            console.log('AI不可用：', typeof AIRootAnalyzer === 'undefined' ? 'AIRootAnalyzer未定义' : '未设置API密钥');
        }
    ...
    // 判断是否需要AI分析（现在所有词都使用AI）
    shouldUseAI(result) {
        return true; // 所有词都使用AI分析
    },
