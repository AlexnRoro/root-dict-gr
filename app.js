...
function displayCombinedResult(data, word) {
    ...
    console.log('词源典故:', data.etymology);
    console.log('相关词汇:', data.related);
    ...
    resultDiv.innerHTML = `
        <div class="word-card">
            ...
            <div class="word-actions">
                ...
                <button onclick="showRootTree('${word}')" class="tree-btn">词根树</button>
            </div>
            <div id="rootTreeContainer" style="display: none;"></div>
            <div id="similarWordsContainer" style="display: none;"></div>
        </div>
    `;
}
...
async function findSimilarWords(word) {
    ...
    if (!container) {
        // 如果容器不存在，创建一个
        const resultDiv = document.getElementById('result');
        const containerHtml = '<div id="similarWordsContainer" style="display: none;"></div>';
        resultDiv.insertAdjacentHTML('beforeend', containerHtml);
    }
    
    const similarContainer = document.getElementById('similarWordsContainer');
    similarContainer.innerHTML = '<div class="loading">正在分析相似词汇...</div>';
    similarContainer.style.display = 'block';
    
    try {
        // 优先使用AI分析
        if (typeof AIRootAnalyzer !== 'undefined' && AIRootAnalyzer.findSimilarWords) {
            const aiSimilar = await AIRootAnalyzer.findSimilarWords(word);
            if (aiSimilar && aiSimilar.length > 0) {
                displaySimilarWordsInline(aiSimilar, word);
                return;
            }
        }
    } catch (error) {
    ...
    for (const [w, data] of Object.entries(wordDatabase)) {
        if (w !== wordLower && (w.includes(wordLower) || wordLower.includes(w))) {
            similar.push({ word: w, similarity: '字符相似', explanation: '包含相同字符' });
        }
    }
        ...
        for (const [w, data] of Object.entries(wordDatabase)) {
            if (w !== wordLower && data.roots) {
                ...
                if (hasCommonRoot && !similar.find(s => s.word === w)) {
                    similar.push({ word: w, similarity: '共享词根', explanation: '包含相同词根' });
                }
            }
        }
    }
    
    displaySimilarWordsInline(similar.slice(0, 10), word);
}

// 内联显示相似词汇（类似词根树）
function displaySimilarWordsInline(similarWords, originalWord) {
    const container = document.getElementById('similarWordsContainer');
    
    if (similarWords.length === 0) {
        container.innerHTML = '<div class="error">未找到相似词汇</div>';
        return;
    }
    
    const similarHtml = `
        <div class="similar-words-tree">
            <div class="tree-title">🔗 ${originalWord} 相似词汇</div>
            <div class="tree-content">
                <div class="target-word">${originalWord}</div>
                <div class="similar-branches">
                    ${similarWords.map(s => `
                        <div class="similar-branch">
                            <div class="similar-node">
                                <span class="similar-word-text" onclick="searchRelatedWord('${s.word}')">${s.word}</span>
                                <span class="similarity-type">${s.similarity}</span>
                            </div>
                            <div class="similarity-desc">${s.explanation}</div>
                        </div>
                    `).join('')}
                </div>
            </div>
            <div class="ai-badge">✨ 由AI智能分析提供</div>
            <button onclick="hideSimilarWords()" class="close-tree-btn">关闭</button>
        </div>
    `;
...
async function showRootTree(word) {
    ...
    container.innerHTML = '<div class="loading">正在生成词根树...</div>';
    ...
    try {
        if (typeof AIRootAnalyzer !== 'undefined' && AIRootAnalyzer.generateRootTree) {
            ...
            if (treeData && treeData.tree) {
                ...
                container.innerHTML = '<div class="error">无法生成词根树</div>';
            }
        } else {
            container.innerHTML = '<div class="error">请先设置AI功能</div>';
        }
    } catch (error) {
        container.innerHTML = '<div class="error">生成词根树失败</div>';
        console.error('词根树错误:', error);
    }
}
...
function displayRootTree(tree) {
    ...
    const treeHtml = `
        <div class="root-tree">
            <div class="tree-title">🌳 ${tree.word} 词根树</div>
            <div class="tree-content">
            ...
            <button onclick="hideRootTree()" class="close-tree-btn">关闭</button>
        </div>
    `;
