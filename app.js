    ...
    const loadingHtml = `
        <div class="similar-modal">
            <div class="similar-content">
                <h3>正在分析与 "${word}" 相似的词汇...</h3>
                <div class="loading">AI正在分析中，请稍候...</div>
                <button onclick="closeSimilarModal()">取消</button>
            </div>
        </div>
    `;
    ...
    try {
        ...
        if (typeof AIRootAnalyzer !== 'undefined' && AIRootAnalyzer.findSimilarWords) {
            ...
            if (aiSimilar && aiSimilar.length > 0) {
                closeSimilarModal();
                displayAISimilarWords(aiSimilar, word);
                return;
            }
        }
    } catch (error) {
...
// 显示AI相似词汇
function displayAISimilarWords(similarWords, originalWord) {
    ...
    const similarHtml = `
        <div class="similar-modal">
            <div class="similar-content">
                <h3>🤖 AI分析：与 "${originalWord}" 相似的词汇</h3>
                <div class="similar-list">
                    ${similarWords.map(s => `
                        <div class="ai-similar-item" onclick="searchSimilarWord('${s.word}')">
                            <div class="similar-word">${s.word}</div>
                            <div class="similarity-reason">🔗 ${s.similarity}</div>
                            <div class="similarity-explanation">${s.explanation}</div>
                        </div>
                    `).join('')}
                </div>
